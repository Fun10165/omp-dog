...
/**
 * Opaque live-agent token handed to the agentic kernel and back to the host
 * layer, which knows its concrete type (a DSH `Agent`). The engine and the
 * kernel contracts never inspect it — that structural opacity is what keeps
 * `src/core/` free of Harness imports.
 */
export interface HostAgentToken {
 readonly id: string
}
