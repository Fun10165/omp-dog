...
export function panelLines(options: {
	...
	for (const goalId of Object.keys(nodes).slice(0, limit)) {
		...
		const verifier =
			node.verifier?.mode === "agentic" ? "agent" : node.verifier?.mode === "programmatic" ? "script" : "composite";
...
function countStates(run: DogRun): string {
	...
	// Both tables are Records over the whole state union, so a state added to the
	// protocol fails to compile until it has a tally slot and a display position.
	// A state that exists in a run can therefore never be dropped from this line.
	return (Object.keys(tally) as GoalState[])
		.filter((state) => tally[state] > 0)
		.toSorted((left, right) => STATE_PRIORITY[left] - STATE_PRIORITY[right])
		.map((state) => `${tally[state]}${GLYPHS[state]}`)
		.join(" ");
}

/** Display order for the stats line; see countStates. */
const STATE_PRIORITY: Record<GoalState, number> = {
	success: 0,
	inherited: 1,
	failure: 2,
	needs_human: 3,
...
export function runReport(options: { readonly run: DogRun; readonly compiled: CompiledGraph | undefined }): string {
	const { run, compiled } = options;
	const lines: string[] = [];
