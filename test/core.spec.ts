...
import type { DogConfig, VerifierShape } from "../core/model.ts";
...
function engine(
	root: string,
	opts: {
		programmatic?: ReturnType<typeof stubProgrammatic>;
		agentic?: ReturnType<typeof stubAgentic>;
		config?: DogConfig;
		agentWorkspaceDir?: (id: string) => string | undefined;
		now?: () => Date;
	} = {},
): DogEngine {
...
describe("v0.9 engine", () => {
	...
	it("captures targets from the invoking session cwd first, with the configured root as fallback", async () => {
		...
		const fromCwd = await dog.create(
			graph(
				{
		...
		const fromRoot = await dog.create(
			graph(
				{
					root: compositeNode({ op: "ref", id: "leaf" }),
		...
		const missing = await dog.create(
			graph(
				{
					root: compositeNode({ op: "ref", id: "leaf" }),
			...
			graph(
				{
					...
					lenient: leafNode({
						title: "lenient",
						constraint: "soft",
						verifier: { mode: "agentic", instruction: "anything" },
					}),
				},
				[
	...
	it("runs programmatic leaves without queuing behind the agentic concurrency budget", async () => {
		...
		const repo = new DogRepository(join(root, ".dog-store"));
		const fastSettled = (await repo.loadGoalRuntimeEvents(run.runId, "fast")).find(
			(event) => event.phase === "goal_settled",
		);
		const slowSettled = (await repo.loadGoalRuntimeEvents(run.runId, "slow")).find(
			(event) => event.phase === "goal_settled",
		);
